...
public class DivisorsCommand implements Command {
    ...
    public Output execute(Input input) {
        try {
            ...
            return isOutOfBounds(number) ? isOutOfBoundOut() : outPutOf(number);
    ...
    private String divisorsOf(int number) {
        return IntStream.rangeClosed(1,Math.abs(number)).filter(i->number%i==0).mapToObj(String::valueOf).collect(Collectors.joining(","));
    }

    private Output isOutOfBoundOut() {
        return new Output() {
            @Override
            public int responseCode() {
                return 404;
